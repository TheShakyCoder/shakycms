...
const props = defineProps({ stat: { type: Object, required: true } });

const form = useForm({
    value: props.stat.value,
    label: props.stat.label,
    icon: props.stat.icon,
    order: props.stat.order,
});

function submit() {
    form.put(`/internal/stats/${props.stat.id}`);
}
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-semibold text-warm-900 font-display">Edit Stat</h1>
                    <p class="text-sm text-warm-500 mt-0.5">Update this homepage statistic</p>
                </div>
            </div>
        </template>
